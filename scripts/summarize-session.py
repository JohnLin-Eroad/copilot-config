...
def fetch_existing_bullets(page_id: str) -> set[str]:
    """Fetch all existing bullet point text from a Notion page (handles pagination)."""
...
def build_summary(messages: list[dict], workspace: dict, session_id: str, prev_stem: str | None = None) -> str:
    """Build a markdown summary from the session data."""
    ...
    # Parse created_at for a nicer date
    if created_at:
        try:
    ...
    # Key topics — first user message and any that look like key decisions
    ...
    for i, msg in enumerate(messages):
        ...
        if role == "user":
            # Show full user messages (they're usually short)
            lines.append(f"**🧑 You:** {content}")
            lines.append("")
        else:
            # For AI messages, show a condensed version
    ...
    lines.append("---")
    lines.append(f"*Auto-generated by `~/.copilot/scripts/summarize-session.py`*")

    # Prepend nav bar just after the front-matter block
    nav = f"**← [[{prev_stem}]]** | → *(next)*" if prev_stem else "← *(first)* | → *(next)*"
    insert_at = lines.index("") + 1  # first blank line after front-matter
