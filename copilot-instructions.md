...
> ## ⚡ SQL-ONLY BRAIN MODE (active 2026-05-19)
>
> The Obsidian vaults (`~/eroad-brain`, `~/john-brain`) are **no longer the source of truth**. The brain is now the SQLite graph at `~/.copilot/brain-graph.db`.
>
> - **Read** via `python3 ~/.copilot/scripts/brain-graph-query.py search|traverse ...`
> - **Write** via SQL upserts into the `nodes` / `edges` tables (see `brain-consolidation.agent.md`)
> - **Do NOT** grep, read, or write `.md` files inside `~/eroad-brain` or `~/john-brain` for brain knowledge
> - `obsidian-sync` and `brain-repo-sync` launchd jobs are unloaded
>
> `.github/learnings.md` per-repo writes are unchanged.

---

## Agent & Skill Dispatch

The orchestrator routes every task through specialist agents. Default to the matrix below; if no fit, invoke `agent-factory.agent.md` — never use `general-purpose` as a fallback.

| Domain | Primary agent file |
|---|---|
| Pipeline routing & STM | `orchestrator.agent.md` |
| Knowledge fetch (SQL brain) | `brain-data-retrieval.agent.md` |
| Knowledge persistence | `brain-consolidation.agent.md` |
| Architecture decisions / ADRs | `architect.agent.md` |
| Implementation (Java/Spring/Next) | `developer.agent.md` |
| Test authoring & validation | `qa-engineer.agent.md`, `testing.agent.md` |
| Code review (high signal) | `code-reviewer.agent.md` |
| Security audit & OWASP | `security.agent.md` |
| DB migrations | `data-migration.agent.md`, `migration-validator.agent.md` |
| CI/CD & infra | `devops.agent.md` |
| Product specs & Jira | `product-manager.agent.md`, `product-owner.agent.md` |
| Strategy / Exec / Finance | `erd-strategy.agent.md`, `erd-executive.agent.md`, `erd-finance.agent.md` |
| Weekly benchmarking | `benchmark-runner.agent.md` |
| Weekly retrospective | `retrospective.agent.md` |
| New capability needed | `agent-factory.agent.md` |

Reusable cross-agent procedures live as **skills** in `~/.copilot/skills/`:

| Skill | Invoke when |
|---|---|
| `skills/brain-sync/SKILL.md` | Start/end of every task — context lookup + write-back (now SQL-backed) |
| `skills/blast-radius/SKILL.md` | Before any HIGH or CRITICAL change |
| `skills/rollback-plan/SKILL.md` | After blast-radius returns MEDIUM+ |
| `skills/dual-critique/SKILL.md` | HIGH/CRITICAL decisions where optimism bias could mislead |
| `skills/critical-thinker/SKILL.md` | Evaluating a plan/proposal/architecture |
| `skills/advisor/SKILL.md` | Strategic or directional decisions (multi-perspective panel) |
| `skills/handoff-protocol/SKILL.md` | When passing context between agents in a pipeline |
| `skills/tdd-workflow/SKILL.md` | Before writing implementation when ACs exist |
| `skills/context-compression/SKILL.md` | STM exceeds ~200KB / quality degrading |
| `skills/session-summary/SKILL.md` | Session ending / "save session" |
| `skills/jira-confluence-sync/SKILL.md` | Creating/updating Jira tickets or Confluence pages |
| `skills/html-report/SKILL.md` | End of task — produce a self-contained results page |
| `skills/unstick/SKILL.md` | Stuck after 3+ failed attempts or 5+ tool calls with no progress |
