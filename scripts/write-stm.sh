...
# Warn when STM exceeds 200KB (~50k tokens). Signal orchestrator to compress.
STM_SIZE_BYTES="$(wc -c < "$STM_PATH" 2>/dev/null | tr -d ' ')"
STM_SIZE_KB=$(( STM_SIZE_BYTES / 1024 ))

if [[ $STM_SIZE_BYTES -gt 204800 ]]; then
    echo "[write-stm] ⚠️  STM is ${STM_SIZE_KB}KB (>${STM_SIZE_BYTES} bytes) — exceeds 200KB threshold"
    echo "[write-stm] 🗜  PIPELINE_SIGNAL: COMPRESS_STM — orchestrator should compress Agent Contributions before next agent spawn"

    # Auto-compress if check-stm-size.sh exists (best-effort, non-blocking)
    COMPRESS_SCRIPT="$(dirname "$0")/check-stm-size.sh"
    if [[ -x "$COMPRESS_SCRIPT" ]]; then
        bash "$COMPRESS_SCRIPT" "$STM_PATH" 2>/dev/null || true
    fi
elif [[ $STM_SIZE_BYTES -gt 153600 ]]; then
    echo "[write-stm] ℹ️  STM is ${STM_SIZE_KB}KB — approaching 200KB compression threshold (75%)"
fi
