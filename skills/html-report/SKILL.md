...
- **Must be fully self-contained** — no external CDNs, Google Fonts, or image URLs. Everything inline. Users open these offline.
- **Don't generate CSS from memory** — always read DETAIL.md. LLMs default to generic blue/white styles that look terrible.
- **Don't hardcode widths** — use responsive CSS. Users resize browsers and save to PDF.
- **Don't use `<img src="...">` with URLs** — if you need images, base64-encode them or use CSS/SVG.
- **Don't over-engineer the layout** — a clear summary page beats a complex dashboard. Match complexity to the task output.
- **Don't forget `<meta charset="utf-8">`** — emoji and special characters break without it.
- **Open immediately** — don't ask "shall I open this?" Just `open <filepath>`.
